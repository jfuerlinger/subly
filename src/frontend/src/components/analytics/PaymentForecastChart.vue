...
import { buildPaymentForecast } from '../../app/utils/subscriptionMath'

const props = defineProps<{ subscriptions: Subscription[] }>()

const data = computed(() => buildPaymentForecast(props.subscriptions))
...
<template>
  <section class="card">
    <h2>Zahlungsvorschau</h2>
    <p class="chart-subtitle muted">Erwartete Ausgaben – nächste 12 Monate</p>

    <p v-if="!hasData" class="muted">Keine aktiven Abos vorhanden.</p>
    ...
    <Teleport to="body">
      ...
      >
        ...
        <span>Zahlungen: {{ formatCurrency(data[hoveredIndex].total) }}</span>
      </div>
    </Teleport>
  </section>
</template>
